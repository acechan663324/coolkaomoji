...
export const Navigation: React.FC<NavigationProps> = ({ onHomeClick }) => {
  ...
  const handleGeneratorClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    }, 100); // A small delay to allow React to re-render the main page
  };
  ...
  const navLinks = [
    { name: 'Home', href: '#', onClick: (e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); onHomeClick(); } },
    ...
    <nav className="bg-white/80 backdrop-blur-md sticky top-0 z-40 w-full border-b border-slate-200">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          <a href="#" onClick={(e) => { e.preventDefault(); onHomeClick(); }} className="flex items-center gap-2" aria-label="Kaomoji World Home">
